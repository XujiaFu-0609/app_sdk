...
#[no_mangle]
pub extern "C" fn app_create_message(sender: *const c_char, content: *const c_char) -> *mut c_char {
...
#[no_mangle]
pub extern "C" fn app_send_message_json(json: *const c_char) -> c_int {
...
#[no_mangle]
pub extern "C" fn app_send_message(sender: *const c_char, content: *const c_char) -> c_int {
    let sender = unsafe { CStr::from_ptr(sender) }.to_string_lossy().into_owned();
    let content = unsafe { CStr::from_ptr(content) }.to_string_lossy().into_owned();
    let m = create_message(&sender, &content);
...
#[no_mangle]
pub extern "C" fn app_string_free(s: *mut c_char) {
    if s.is_null() { return; }
